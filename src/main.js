...
import { aliasToModule, moduleToPackage } from './hljs-map.js'
import { coreImporters } from './hljs-importers.js'
import './style.css'
...
const log = (...args) => { if (DEBUG_HL) console.debug('[hl]', ...args) }
...
const registerLang = async modName => {
    ...
    const importer = pkg
        ...
        : moduleToImporter[modName]
    if (!importer) {
        log('no importer for', modName, '(pkg:', pkg ?? 'core', ')')
        return
    }
    const mod = await importer().catch(err => { log('import failed for', modName, err); return null })
    if (!mod) return
