...
const copySmart = async (text, notify) => {
    ...
    return navigator.clipboard.writeText(text).then(() => notify('copied to clipboard')).catch(fallback)
}
...
export const initUI = ({ getMarkdown, setMarkdown }) => {
    ...
    const buttons = [
        ...
        [
            'load-from-file',
            'load',
            'gravity-ui:folder-open-fill', /* 'basil:folder-open-solid', */
            async () => {
    ...
    // set initial spell state and keep in sync with editor mount
